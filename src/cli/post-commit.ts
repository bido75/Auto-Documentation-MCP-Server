...
export async function runPostCommitTrigger(env = process.env): Promise<void> {
  ...
  console.error(
    `[auto-doc-mcp] post-commit result: ok=${result.ok} projectId=${result.projectId} repoPath=${result.repoPath} mode=${result.mode}`,
  );
}
