...
export async function startHttpBridge(options?: { port?: number; host?: string }): Promise<void> {
  ...
  app.get("/sse", async (req: Request, res: Response) => {
    ...
    if (!sessionToken || sessionToken.length === 0) {
      res.status(401).json({
        ok: false,
        error: "Missing Notion token. Provide x-notion-token or set NOTION_TOKEN.",
      });
      return;
    }
    ...
    sessionNotionTokens[sessionId] = sessionToken;
    transport.onclose = () => {
    ...
    await runWithRuntimeContext({ notionToken: sessionToken }, async () => {
  ...
  await new Promise<void>((resolve) => {
    app.listen(port, host, () => {
      console.log(`Auto-Doc MCP HTTP bridge running on http://${host}:${port}`);
      console.log(`Connect web tools to http://${host}:${port}/sse`);
