...
export class DeterministicProvider implements ModelProvider {
  ...
  async analyze(ev: StructuredEvidence): Promise<ModelAnalysis> {
    ...
    const audiences: ModelAnalysis["audiences"] = [];
    ...
    if (ev.uiComponents.length > 0) {
      score += 10;
    }
    if (ev.apiEndpoints.length > 0) {
      score += 10;
    }
    if (ev.testStatus === "passed") {
      score += 10;
    }
    if (ev.prTitle) {
      score += 10;
    }
    if (ev.dbMigrations.length > 0) {
      score += 5;
    }
    ...
    return {
      ...
      reviewQuestions:
        score < 75
          ? ["What is the intended user-facing behavior?", "Which user roles can access this feature?"]
          : [],
