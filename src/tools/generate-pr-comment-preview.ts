...
type ManualAudience = "User" | "Admin" | "Both" | "Internal";
...
function matchesAudience(audience: ManualAudience, filter: AudienceFilter): boolean {
  if (filter === "both") {
    return true;
  }
...
export function registerGeneratePrCommentPreviewTool(server: McpServer) {
  server.tool(
    "generate_pr_comment_preview",
    "Builds a markdown preview payload suitable for a GitHub PR comment.",
    {
      projectId: z.string(),
    ...
    async (input) => {
      ...
      try {
        ...
        const previewEntries = records
          .map((record) => {
            ...
            return {
              ...
              audience: (getSelectName(properties, "Audience") ?? "Internal") as ManualAudience,
        ...
        const statusCounts = selectedEntries.reduce<Record<string, number>>((acc, entry) => {
