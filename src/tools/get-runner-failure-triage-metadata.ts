...
import {
  getStateStore,
  type RunnerFailureTriageHistoryEntry,
  type RunnerFailureTriageMetadata,
...
function findLastAcknowledgement(history: RunnerFailureTriageHistoryEntry[]) {
...
function findLastCooldownChange(history: RunnerFailureTriageHistoryEntry[]) {
...
function toTimelineEvent(entry: RunnerFailureTriageHistoryEntry): TimelineEvent {
    return {
        changedAt: entry.changedAt,
...
function filterTimelineEventsByLabels(events: TimelineEvent[], timelineLabels: TimelineLabel[]): TimelineEvent[] {
...
export function registerGetRunnerFailureTriageMetadataTool(server: McpServer): void {
    server.tool("get_runner_failure_triage_metadata", "Reads current runner failure triage metadata and recent metadata update history for a project and repository target.", {
        ...
        const resolvedHistoryView = historyView ?? "all";
        const resolvedSortOrder = sortOrder ?? "desc";
        const resolvedResponseMode = responseMode ?? "standard";
