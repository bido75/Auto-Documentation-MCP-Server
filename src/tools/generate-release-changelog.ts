...
async function loadEntryBodySummary(notion: ReturnType<typeof createNotionClient>, pageId: string): Promise<string> {
...
export function registerGenerateReleaseChangelogTool(server: McpServer) {
  server.tool(
    "generate_release_changelog",
    "Generates a release changelog markdown from approved/published manual entries linked to a release.",
    {
      projectId: z.string(),
    ...
    async ({ projectId, releaseVersion, maxEntries, traceId: incomingTraceId }) => {
      ...
      try {
        ...
        const manualFilters: Array<Record<string, unknown>> = [
          {
            property: "Project",
            relation: { contains: projectPageId },
          },
