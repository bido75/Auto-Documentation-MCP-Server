...
export function registerRunReleaseDocumentationPipelineTool(server: McpServer) {
    server.tool("run_release_documentation_pipeline", "Runs release-tag documentation automation: capture, changelog, package, PDF export, local sync, and optional PR comment posting.", {
        ...
        try {
            ...
            const pdfResult = parseToolText(await exportPdf({
                projectId,
                releaseVersion,
                audience,
            ...
            const syncResult = parseToolText(await syncLocalDocs({
                projectId,
                audience,
                releaseVersion,
            ...
            const helpCenterResult = helpCenterOutputPath
                ? parseToolText(await exportHelpCenter({
                    projectId,
                    audience,
                    releaseVersion,
            ...
            const prCommentResult = prUrl
                ? parseToolText(await publishPrComment({
                    projectId,
                    prUrl,
                    audience,
                    traceId,
