...
function parsePullRequestUrl(prUrl: string): GithubPrTarget {
...
export function registerPublishPrCommentTool(server: McpServer) {
  server.tool(
    "publish_pr_comment",
    "Publishes or updates an auto-documentation preview comment on a GitHub pull request.",
    {
      projectId: z.string(),
      prUrl: z.string().url(),
    ...
    async ({ projectId, prUrl, audience, maxEntries, dryRun, traceId: incomingTraceId }) => {
      ...
      try {
        ...
        const preview = parseToolText<PrCommentPreviewResult>(
