...
async function main() {
  ...
  try {
    ...
    if (process.env.WEBHOOK_SECRET) {
      const payload = JSON.stringify({ zen: 'functional smoke ping' });
      const sig = crypto.createHmac('sha256', process.env.WEBHOOK_SECRET).update(payload).digest('hex');
      const candidates = ['/webhooks/github', '/webhook', '/webhooks', '/webhook/github', '/mcp/webhook'];
      let hookPath = null;
      ...
      if (hookPath) {
        const good = await req('POST', `${base}${hookPath}`, {
          headers: { 'content-type': 'application/json', 'x-hub-signature-256': `sha256=${sig}`, 'x-github-event': 'ping' }, body: payload });
        ...
        const bad = await req('POST', `${base}${hookPath}`, {
          headers: { 'content-type': 'application/json', 'x-hub-signature-256': 'sha256=deadbeef', 'x-github-event': 'ping' }, body: payload });
