...
exports.createLoadingStatusPanelState = createLoadingStatusPanelState;
exports.createReadyStatusPanelState = createReadyStatusPanelState;
exports.createErrorStatusPanelState = createErrorStatusPanelState;
exports.buildStatusPanelHtml = buildStatusPanelHtml;
function createLoadingStatusPanelState(message = "Loading documentation status...") {
    return {
        kind: "loading",
        message,
    };
}
function createReadyStatusPanelState(status, refreshedAt = new Date().toISOString()) {
    const missingFields = ["health", "publishedCount", "needsReviewCount", "capturedCount", "lowConfidenceCount"].filter((key) => status[key] === undefined);
    return {
        kind: "ready",
        status,
        refreshedAt,
...
function createErrorStatusPanelState(message, options = {}) {
    return {
        kind: "error",
        message,
...
function renderForcedQueueReasons(reasons) {
    if (reasons.length === 0) {
...
function buildStatusPanelHtml(state) {
    ...
    const panelBody = state.kind === "loading"
        ...
        : state.kind === "error"
            ? `<div class="callout error"><strong>Status unavailable</strong><p>${escapeHtml(state.message)}</p>${state.details ? `<pre>${escapeHtml(state.details)}</pre>` : ""}</div>`
...
<body>
  ...
  <div class="meta">${refreshedAt ? `Last refreshed ${escapeHtml(refreshedAt)}` : "Waiting for status response..."}</div>
  ${panelBody}
